...
`pagination-url`, `page-object`, `pagination-qualifier`, `page-size-selector` and `pagination-onclick` (NEW!).
...
`sd:pagination-onclick` can be used to set a JavaScript function to execute upon clicking an item from code generated by full pagination. This makes it possible, for example, to update the DOM dynamically, making it unnecessary to reload the entire page. The passed function must handle _one argument_ - a string corresponding to the URL of the desired page. Calls to a function `updateDynamically`, for instance, would be made as follows: `updateDynamically('/example/url?page=n')`.

```html
<nav>
    <ul class="pagination" sd:pagination="full" sd:pagination-onclick="updateDynamically">
        <!-- Pagination created by SpringDataDialect, this content is just for mockup -->
        <li class="disabled"><a href="#" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
        <li class="active"><a href="#">1 <span class="sr-only">(current)</span></a></li>
    </ul>
